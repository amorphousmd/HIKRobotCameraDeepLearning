...
# A new class that modifies the BaseEngine class
...
# Create a list of centers of the bounding boxes
# Requires the coord_list and the confidence score
...
class Logic(QMainWindow, Ui_MainWindow):
    ...
    def cuda_contextYOLO(self):
        ...
        pred = BaseEngineCracker(engine_path='./Weights/Crackers2YOLOv7.trt')

        while True:
            # This line is blocking until a new frame is grabbed
            self.img = obj_cam_operation.get_np_image()
            self.time_start = time.time()
            self.imgSave = self.img
            try:
